...
function SelectFolderModal({ onClose, onSelect, currentPath, fileName }) {
  ...
  const fetchFolders = async (folderPath = "") => {
    try {
      ...
      if (isSharedValue) {
        if (folderPath === "") {
          params = { limit: 1000, shared: filenameRedux };
        } else {
          params = { folderPath, shared: filenameRedux };
        }
      } else {
        if (folderPath === "") {
          params = { limit: 1000 };
      ...
      const res = await axios.get(
        folderPath === "" ? `${apiUrl}getAllObjectsNew` : `${apiUrl}getFolder`,
        {
          params,
      ...
      const folders = folderPath === ""
        ? res.data.result.filter((item) => item.isFolder === true)
        : res.data.filter((item) => item.isFolder === true);
          ...
          <div className="modal-content" style={{ borderRadius: "12px" }}>
            ...
            <div className="modal-body" style={{ padding: "20px 24px" }}>
              <p style={{ marginBottom: "16px", color: "#666", fontSize: "14px" }}>
                Select a folder to unzip <strong>{fileName}</strong> to:
              </p>
