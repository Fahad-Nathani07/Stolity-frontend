...
import { getFilteredFullFileList } from "../utils/audioPlayer";
...
const handleMulDownload = async () => {
  ...
  //       if (isGoogleAuth) {
  //         combinedData = [...sharedFolders, ...files];
  //       } else {
  //         if (
    ...
    try {
      if (isFolder) {
        try {
          ...
          if (err?.name === "AbortError" || isDownloadCancelledError(err)) {
            throw err;
          }
          console.warn(
            "[Favourites] No-zip folder download failed, falling back to zip:",
            err?.message || err
          );
...
function getModifiedRecentFolderText(input) {
  ...
  //         fileSize: file.size,
  //         filePath: path, // Base folder path
  //         localPath,
  //         acl: pubpri === "private" ? "private" : "public-read",
  //       };
  //     });

  //     // 🔁 Step 1: Get presigned URLs in one call
  ...
  //     // 🔁 Step 2: Upload each file using its respective URL
...
// };
  ...
  //     while (!fileType && filedata.length > 0) {
  ...
  //       setVideoSrc("");
  //       setPdfSrc("");
  //       getAudioInfo(filedata[newIndex].fileName);
...
const handleNext = () => {
  ...
  const moveDraggedFile = async (filename) => {
    if (filename.isFolder === true) {
      ...
      addUpload(uploadId, "Moving " + movingFile, { operation: "move" });
      try {
        const res = await axios.post(
          `${apiUrl}move-file`,
          {
  ...
  const moveMultipleDrag = async (arr, folname) => {
    try {
      ...
      addUpload(uploadId, "Moving Files..", { operation: "move" });
  ...
  const moveMultipleDrag2 = async (arr, folname) => {
    ...
    try {
      if (arr && arr.length > 0) {
        ...
        addUpload(uploadId, "Moving " + movingFolders, { operation: "move" });
          ...
          <div className="content-wrapper">
            <div className={tableBoxClassName} ref={tableBoxRef}>
              ...
              <StoreBulkSelectionToolbar
                ...
                onCopy={() => setIsCWhisperClicked(true)}
  ...
  Convert Files (Optional)
      ...
      {showDeleteModal && (
        <div className="rename_popup_wrapper">
          <div className="rename_modal">
            <h2 className="rename_title2" style={{ marginBottom: 24 }}>
              Are you sure you want to delete all the selected files and folders?
            </h2>
