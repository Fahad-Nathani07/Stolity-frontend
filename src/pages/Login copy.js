...
import { setGoogleAuth } from "../store/fileSlicer"; // adjust path as needed
...
const Login = ({ setSpanExpanded, spanExpanded }) => {
  ...
  // ────────────────────────────────────────────────
  //   Google login handlers (existing)
  // ────────────────────────────────────────────────

const handleSignUpSuccess = (response) => {
  axios
    .post(`${apiEndPoint}firebaseAuth`, {
    ...
    .then((res) => {
      ...
      setSpanExpanded(true);
      console.log("aaaaa Google Login Response: ", data);

      const a = data.accessToken;
      const a2 = data.loginId;
      const b = data.userData;
      const c = data.userData.email;
      const rawAvatar = normalizeAvatarUrl(data.userData?.userAvatar);
      ...
      // sessionStorage.setItem("avatar", d);
...
<form onSubmit={handleSubmit}>
  <div className="mt-4">
      ...
      <>
        ...
        <div className="btn_login_group">
          <button
            ...
            style={isLoggingIn ? { opacity: 0.75, cursor: "not-allowed" } : undefined}
          >
    ...
    <GoogleOAuthProvider clientId={clientId}>
      <div style={{ marginTop: "10px", justifySelf: "center" }}>
        <GoogleLogin
